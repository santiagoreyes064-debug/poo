...
function TabsList({ children, className, activeValue, onValueChange }: TabsListProps) {
  return (
    <div className={cn('inline-flex h-10 items-center justify-center rounded-md bg-gray-800 p-1', className)}>
      {React.Children.map(children, (child) => {
        if (React.isValidElement(child)) {
  ...
  return (
    <button
      className={cn(
        ...
        isActive ? 'bg-gray-900 text-gray-100 shadow-sm' : 'text-gray-400 hover:text-gray-100',
        className,
      )}
